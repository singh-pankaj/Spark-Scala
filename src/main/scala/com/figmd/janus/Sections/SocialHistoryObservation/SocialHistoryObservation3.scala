...
import com.figmd.janus.util.{ConditionsUtil, Social_History_Observation_M_TobaccoUser_New_Crosswalk}
import org.apache.spark.sql.{DataFrame, Dataset, Row, SparkSession}

object SocialHistoryObservation3 extends ConditionsUtil with Serializable {
  //Social History Observation_M_TobaccoUser_New

  def process(lookupDf: DataFrame, spark: SparkSession): Dataset[Social_History_Observation_M_TobaccoUser_New_Crosswalk] = {
    ...
    val dmRdd = lookupDf1.map(r => {
      ...
      Social_History_Observation_M_TobaccoUser_New_Crosswalk(r.getString(r.fieldIndex("visituid")), CurntTobcNUsr, CurntTobcNUsr_Date, TobNU, TobNU_Date, TobcUTC, TobcUTC_Date, ToUs_1, ToUs_1_Date)
    }
    ).distinct();
